...
function buildQueryVariations(med) {
  ...
  let dosage = String(med.dosage || "").trim();
  const salt = med.salt || "";
  const normalizedSalt = med.normalized_salt || "";
  const primarySaltKey = med.primary_salt_key || "";

  // Infer dosage from bare numbers in the name when dosage field is empty.
  // e.g., "pyrigesic 1000 tablet" → infer "1000mg"
  // This enables the critical brand+dosage query variation for slug guessing.
  if (!dosage) {
    const bareMatch = name.match(/\b(\d{2,5})\b(?!\s*(?:mg|ml|mcg|g|gm|kg|%))/i);
    if (bareMatch) {
      const num = parseInt(bareMatch[1], 10);
      const PACK_SIZES = new Set([10, 12, 14, 15, 20, 21, 28, 30, 60, 90, 100]);
      if (num >= 25 && num <= 10000 && !PACK_SIZES.has(num)) {
        dosage = `${num}mg`;
      }
    }
  }
