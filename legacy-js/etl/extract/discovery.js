...
async function searchPlatformViaWeb(query, platform, options = {}) {
  ...
  // Use unquoted query so search engines can fuzzy-match against page titles
  // (e.g., "pyrigesic 1000 tablet" matches Apollo's "Pyrigesic 1000 mg Tablet")
...
async function discoverApolloUrls(query, options = {}) {
  ...
  try {
    ...
    const candidateSlugs = new Set([slug, `${slug}-tablets`, `${slug}-tablet`, `${slug}-capsules`]);

    // Smart slug generation: decompose query into brand + dosage + form
    // and generate Apollo's canonical pattern: {brand}-{dosage}{unit}-{form}
    // e.g., "pyrigesic 1000mg" → "pyrigesic-1000mg-tablet"
    const dosageMatch = query.match(/(\d+(?:\.\d+)?)\s*(mg|ml|mcg|gm?|kg|%)/i);
    const formMatch = query.match(/\b(tablet|capsule|syrup|injection|cream|gel|spray|drop|ointment|suspension|inhaler)s?\b/i);
    if (dosageMatch) {
      const dosagePart = `${dosageMatch[1]}${dosageMatch[2].toLowerCase()}`;
      // Strip dosage and form from query to get brand
      let brandPart = query.toLowerCase()
        .replace(/\d+(?:\.\d+)?\s*(?:mg|ml|mcg|gm?|kg|%)/gi, "")
        .replace(/\b(?:tablet|tablets|capsule|capsules|syrup|injection|cream|gel|spray|drop|drops|ointment|suspension|inhaler)s?\b/gi, "")
        .replace(/[^a-z0-9\s-]/g, "")
        .replace(/\s+/g, "-")
        .replace(/-+/g, "-")
        .replace(/^-|-$/g, "")
      ...
      if (brandPart) {
        const formWord = formMatch ? formMatch[1].toLowerCase() : "tablet";
        // Apollo canonical: {brand}-{dosage}-{form}
        candidateSlugs.add(`${brandPart}-${dosagePart}-${formWord}`);
        candidateSlugs.add(`${brandPart}-${dosagePart}-${formWord}s`);
        // Also try without form (some Apollo URLs omit it)
        candidateSlugs.add(`${brandPart}-${dosagePart}`);
      }
    }

    // Also try: if query has a bare number, generate slug with "mg" appended
    // e.g., "pyrigesic 1000 tablet" → try "pyrigesic-1000mg-tablet"
    if (!dosageMatch) {
      const bareNumberMatch = query.match(/(\d{2,5})/);
      if (bareNumberMatch) {
        const num = bareNumberMatch[1];
        let brandPart = query.toLowerCase()
          .replace(/\b\d{2,5}\b/g, "")
          .replace(/\b(?:tablet|tablets|capsule|capsules|syrup|injection|cream|gel|spray|drop|drops|ointment|suspension|inhaler)s?\b/gi, "")
          .replace(/[^a-z0-9\s-]/g, "")
        ...
        if (brandPart) {
          const formWord = formMatch ? formMatch[1].toLowerCase() : "tablet";
          candidateSlugs.add(`${brandPart}-${num}mg-${formWord}`);
          candidateSlugs.add(`${brandPart}-${num}mg-${formWord}s`);
          candidateSlugs.add(`${brandPart}-${num}mg`);
          // Also try with "gm" for 1000+ (Apollo uses "1gm" for 1000mg sometimes)
          if (parseInt(num, 10) >= 1000 && parseInt(num, 10) % 1000 === 0) {
            const gmValue = parseInt(num, 10) / 1000;
            candidateSlugs.add(`${brandPart}-${gmValue}gm-${formWord}`);
          }
        }
      }
    }
