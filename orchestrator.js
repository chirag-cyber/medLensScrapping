...
async function orchestrate(config) {
  ...
  // Load or init state
  let state = config.resume ? loadState() : null;
  if (state && state.mode === config.mode) {
    log(`♻️  Resuming from batch index ${state.currentIndex} (${state.batchesCompleted} batches already done)`);
  } else {
